...
import hs.kr.entrydsm.global.annotation.exceptions.AnnotationException
...
object SpecificationProvider {
     ...
     * @param T 명세 클래스 타입
     * @throws AnnotationException 어노테이션이 없거나 인터페이스를 구현하지 않은 경우
     */
     ...
     * @return 검증 결과
     * @throws AnnotationException 명세를 찾을 수 없는 경우
     */
     ...
     * @param T 검증 대상 객체의 타입
     * @return 결합된 복합 명세
     * @throws AnnotationException 결합할 명세가 없거나 명세를 찾을 수 없는 경우
     */
    fun <T> combineSpecifications(
        ...
        if (names.isEmpty()) {
            throw AnnotationException.combineSpecificationsEmpty()
        }
     ...
     * @param T 대상 객체의 타입
     * @return 명세 인스턴스
     * @throws AnnotationException 명세를 찾을 수 없거나 중복 구현체가 있는 경우
     */
    ...
    fun <T> getSpecificationByName(name: String): SpecificationContract<T> {
        return specificationCache.getOrPut(name) {
            val specificationClasses = specificationRegistry[name]
                ?: throw AnnotationException.specificationNotFound(name)
            
            if (specificationClasses.size > 1) {
                throw AnnotationException.multipleImplementations(
                    name = name,
                    implementations = specificationClasses.map { it.simpleName }
                )
            }
     ...
     *
     * @param specificationClass 검증할 명세 클래스
     * @throws AnnotationException @Specification 어노테이션이 없는 경우
     */
    fun validateRuleAnnotation(specificationClass: Class<*>) {
        specificationClass.getAnnotation(Specification::class.java)
            ?: throw AnnotationException.annotationMissing(
                annotationType = "Specification",
                className = specificationClass.simpleName
            )
    }
     ...
     *
     * @param specificationClass 검증할 명세 클래스
     * @throws AnnotationException SpecificationContract 인터페이스를 구현하지 않은 경우
     */
    fun validateSpecificationContract(specificationClass: Class<*>) {
        if (!SpecificationContract::class.java.isAssignableFrom(specificationClass)) {
            throw AnnotationException.contractNotImplemented(
                className = specificationClass.simpleName,
                contractType = "SpecificationContract"
            )
        }
    }
