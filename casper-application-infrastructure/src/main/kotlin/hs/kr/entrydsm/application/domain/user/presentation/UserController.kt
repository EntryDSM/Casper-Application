...
class UserController(
    ...
    fun createUser(@RequestBody request: CreateUserRequest?): ResponseEntity<CreateUserResponse> {
        return try {
            ...
            val response = CreateUserResponse(
                success = true,
                data = CreateUserResponse.UserData(
    ...
    @GetMapping("/{userId}")
    fun getUserById(@PathVariable userId: String?): ResponseEntity<UserDetailResponse> {
        return try {
            ...
            val response = UserDetailResponse(
                success = true,
                data = UserDetailResponse.UserDetailData(
                    userId = result.userId,
                    name = result.name,
                    phoneNumber = result.phoneNumber,
                    email = result.email,
    ...
    fun getAllUsers(): ResponseEntity<UsersListResponse> {
        return try {
            ...
            val response = UsersListResponse(
                success = true,
                data = UsersListResponse.UsersData(
