...
/**
 * 사용자 삭제 이벤트를 수신하여 연관된 성적을 삭제하는 Consumer 클래스입니다.
 *
 * 사용자 서비스에서 사용자가 삭제되었을 때 해당 사용자의 성적도 함께 삭제하여
...
class DeleteScoreConsumer(
    ...
    /**
     * 사용자 삭제 이벤트를 수신하여 연관된 성적을 삭제합니다.
     *
     * DELETE_USER 토픽에서 접수번호를 수신하고, 해당 접수번호에 해당하는
     * 성적을 삭제합니다. 처리 과정에서 발생하는 오류는 로그로 기록됩니다.
     *
     * @param message 사용자 삭제 이벤트 메시지 (접수번호가 JSON 형태로 포함)
     */
    @KafkaListener(
        topics = [KafkaTopics.DELETE_USER],
        groupId = "delete-score",
        containerFactory = "kafkaListenerContainerFactory",
    )
    fun deleteScore(message: String) {
        try {
            val receiptCode = mapper.readValue(message, Long::class.java)
            logger.info("사용자 삭제로 인한 성적 삭제 시작: receiptCode=$receiptCode")

            scoreConsumeContract.consumeDeleteScore(receiptCode)

            logger.info("성적 삭제 완료: receiptCode=$receiptCode")
        } catch (e: Exception) {
            logger.error("성적 삭제 처리 실패: $message", e)
        }
    }
}
