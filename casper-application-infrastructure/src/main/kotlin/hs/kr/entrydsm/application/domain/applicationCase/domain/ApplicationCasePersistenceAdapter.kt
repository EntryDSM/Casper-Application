package hs.kr.entrydsm.application.domain.applicationCase.domain
...
import hs.kr.entrydsm.application.domain.applicationCase.domain.mapper.GraduationCaseMapper
import hs.kr.entrydsm.application.domain.applicationCase.domain.mapper.QualificationCaseMapper
import hs.kr.entrydsm.application.domain.applicationCase.domain.repository.GraduationCaseJpaRepository
import hs.kr.entrydsm.application.domain.applicationCase.domain.repository.QualificationCaseJpaRepository
...
import hs.kr.entrydsm.application.domain.applicationCase.spi.ApplicationCasePort
...
class ApplicationCasePersistenceAdapter(
    ...
    override fun queryAllApplicationCaseByReceiptCode(receiptCodeList: List<Long>): List<ApplicationCase?> {
        val graduationCaseList =
            graduationCaseJpaRepository.findAllByReceiptCodeIn(receiptCodeList)
                .map { graduationCaseMapper.toDomain(it) }
                .associateBy { it?.receiptCode }

        val qualificationCaseList =
            qualificationCaseJpaRepository.findAllByReceiptCodeIn(receiptCodeList)
