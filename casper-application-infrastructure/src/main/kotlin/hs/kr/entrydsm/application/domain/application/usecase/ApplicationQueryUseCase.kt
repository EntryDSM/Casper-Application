...
class ApplicationQueryUseCase(
    ...
    fun getApplicationById(applicationId: String): ApplicationDetailResponse {
        ...
        val application = applicationRepository.findById(uuid)
            .orElseThrow { IllegalArgumentException("원서를 찾을 수 없습니다: $applicationId") }
        
        return ApplicationDetailResponse(
            success = true,
            data = ApplicationDetailResponse.ApplicationDetailData(
    ...
    fun getApplications(
        ...
        val pagedApplications = if (startIndex < totalElements) {
        ...
        return ApplicationListResponse(
            success = true,
            data = ApplicationListResponse.ApplicationListData(
                applications = pagedApplications.map { app ->
    ...
    fun getApplicationScores(applicationId: String): ApplicationScoresResponse {
        ...
        return ApplicationScoresResponse(
            success = true,
            data = ApplicationScoresResponse.ScoresData(
    ...
    fun getCalculationResult(applicationId: String): CalculationResponse {
        val uuid = UUID.fromString(applicationId)
        val calculationResult = calculationResultRepository.findLatestByApplicationId(uuid)
        ...
        return CalculationResponse(
            success = true,
            data = CalculationResponse.CalculationData(
                ...
                formulaExecution = CalculationResponse.FormulaExecutionDetail(
                    steps = steps.map { step ->
                        CalculationResponse.CalculationStepDetail(
                            ...
                            variables = if (step.variablesUsed != null) {
    ...
    fun getCalculationHistory(applicationId: String): CalculationHistoryResponse {
        ...
        return CalculationHistoryResponse(
            success = true,
            data = CalculationHistoryResponse.HistoryData(
                applicationId = applicationId,
                calculations = calculationResults.map { result ->
    ...
    private fun reconstructNestedScores(scoreEntities: List<hs.kr.entrydsm.application.domain.application.domain.entity.ApplicationScoreJpaEntity>): Map<String, Any> {
        ...
        scoreEntities.forEach { scoreEntity ->
            ...
            val value = when (scoreEntity.scoreType) {
    ...
    private fun extractBreakdownFromSteps(steps: List<hs.kr.entrydsm.application.domain.application.domain.entity.CalculationStepJpaEntity>): Map<String, Double> {
