...
class ApplicationPersistenceService(
    ...
    fun saveApplication(userId: UUID, applicationData: Map<String, Any>): ApplicationJpaEntity {
        val receiptCode = generateReceiptCode()
        
        val entity = ApplicationJpaEntity(
    ...
    fun saveScores(applicationId: UUID, scores: Map<String, Any>) {
    ...
    fun saveCalculationResult(
        ...
        val entity = CalculationResultJpaEntity(
    ...
    private fun flattenAndCreateScoreEntities(applicationId: UUID, scores: Map<String, Any>): List<ApplicationScoreJpaEntity> {
        val entities = mutableListOf<ApplicationScoreJpaEntity>()
        
        fun flatten(map: Map<String, Any>, prefix: String = "") {
            map.forEach { (key, value) ->
                val fullKey = if (prefix.isEmpty()) key else "${prefix}.${key}"
                when (value) {
                    is Map<*, *> -> flatten(value as Map<String, Any>, fullKey)
                    else -> {
                        entities.add(ApplicationScoreJpaEntity(
    ...
    private fun createStepEntities(calculationId: UUID, steps: List<StepInfo>): List<CalculationStepJpaEntity> {
    ...
    private fun extractStringValue(data: Map<String, Any>, key: String): String? {
        return try {
