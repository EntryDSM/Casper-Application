package hs.kr.entrydsm.application.domain.graduationInfo.domain
...
import hs.kr.entrydsm.application.domain.graduationInfo.domain.mapper.GraduationMapper
import hs.kr.entrydsm.application.domain.graduationInfo.domain.mapper.QualificationMapper
import hs.kr.entrydsm.application.domain.graduationInfo.domain.repository.GraduationJpaRepository
import hs.kr.entrydsm.application.domain.graduationInfo.domain.repository.QualificationJpaRepository
...
import hs.kr.entrydsm.application.domain.graduationInfo.spi.GraduationInfoPort
...
class GraduationInfoPersistenceAdapter(
    ...
    override fun queryAllGraduationByReceiptCode(receiptCodeList: List<Long>): List<GraduationInfo?> {
        val graduations =
            graduationJpaRepository.findAllByReceiptCodeIn(receiptCodeList)
                .map { graduationMapper.toDomain(it) }
                .associateBy { it?.receiptCode }

        val qualifications =
            qualificationJpaRepository.findAllByReceiptCodeIn(receiptCodeList)
