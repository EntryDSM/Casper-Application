...
class ApplicationValidationService(
    ...
    fun validateScoresData(
        ...
        try {
            // applicationType 추출 (기본값: COMMON)
            val applicationTypeStr = applicationData["applicationType"] as? String ?: "COMMON"
            val applicationType = try {
                ApplicationType.valueOf(applicationTypeStr)
            } catch (e: IllegalArgumentException) {
                ApplicationType.COMMON
            }

            // ScoreCalculator를 실행하여 검증
            // 계산이 성공하면 데이터가 유효한 것으로 판단
            ...
            // ScoreCalculator에서 발생한 예외를 IllegalStateException으로 변환
            throw IllegalStateException(e.message ?: "성적 데이터 검증 실패", e)
        }
    }
    /**
     * 원서 생성 요청 데이터를 검증합니다.
