...
class DocumentationConventionPlugin : Plugin<Project> {
    ...
    private fun registerDocTasks(project: Project) {
        ...
        val checkAllDocsTaskName = "checkAllDocs"
        if (project.tasks.findByName(checkAllDocsTaskName) == null) {
            project.tasks.register(checkAllDocsTaskName) {
            ...
            project.logger.lifecycle("${project.name}에 ${checkAllDocsTaskName} 태스크를 등록했습니다.")
        }
        
        // 프로젝트 평가 후에 check 태스크가 있으면 checkAllDocs를 의존성으로 추가
        project.afterEvaluate {
            project.tasks.findByName("check")?.dependsOn(checkAllDocsTaskName)
            project.logger.lifecycle("${project.name}의 check 태스크에 ${checkAllDocsTaskName} 의존성을 추가했습니다.")
        }
